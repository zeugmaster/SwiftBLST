...
@inline(__always)
func withOptionalBytes<R>(_ bytes: [UInt8]?, _ body: (UnsafePointer<UInt8>?, Int) throws -> R)
  rethrows -> R
{
  guard let bytes else { return try body(nil, 0) }
...
public struct SecretKey: Sendable, Equatable {
  ...
  public init(bytes: [UInt8]) throws {
    guard bytes.count == 32 else { throw BLSError.invalidLength(expected: 32, actual: bytes.count) }
    var sk = blst_scalar()
  ...
  public func signG2(
    message: [UInt8], domainSeparationTag dst: String = BLSScheme.basic, augmentation: [UInt8] = [],
...
public struct PublicKeyG1: Sendable, Equatable {
  ...
  public init(compressed bytes: [UInt8], subgroupCheck: Bool = true) throws {
    guard bytes.count == 48 else { throw BLSError.invalidLength(expected: 48, actual: bytes.count) }
  ...
  public init(uncompressed bytes: [UInt8], subgroupCheck: Bool = true) throws {
    guard bytes.count == 96 else { throw BLSError.invalidLength(expected: 96, actual: bytes.count) }
    var p = blst_p1_affine()
    try bytes.withUnsafeBufferPointer {
